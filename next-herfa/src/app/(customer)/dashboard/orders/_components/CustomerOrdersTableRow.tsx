            ...
            {/* <td className="whitespace-nowrap px-6 py-5 text-sm text-gray-500">
                <div className="flex items-center justify-center">
                    {order.status === "pending" ? (
                        <button
                            className="flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-red-600 transition-colors hover:bg-red-50 dark:text-red-500 dark:hover:bg-red-900/20"
                            onClick={(e) => {
                                e.stopPropagation();
                        ...
                        <div className="flex items-center justify-center gap-2 rounded-lg bg-green-50 px-3 py-2 font-semibold text-green-600 dark:bg-green-900/20 dark:text-green-500">
                            <CheckCircle size={16} />
                            <span className="text-sm">Accepted</span>
                        </div>
                    ) : order.status === "canceled" ? (
                        <div className="flex items-center justify-center gap-2 rounded-lg bg-red-50 px-3 py-2 font-medium text-red-600 dark:bg-red-900/20 dark:text-red-500">
                            <XCircle size={16} />
                            <span className="text-sm">Canceled</span>
                        </div>
                    ) : (
                        <Link
                            className="flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-blue-600 transition-colors hover:bg-blue-50 dark:text-blue-500 dark:hover:bg-blue-900/20"
                            href={`/customer-dashboard/worker/${encodeURIComponent(encryptId(order.worker.id))}`}
